...
def sort_conformers_by_energy(input_sdf, output_sdf):
    ...
    for mol in suppl:
        ...
        for conf_id in range(mol.GetNumConformers()):
            ...
            conformer_energies.append((mol, conf_id, energy))

    conformer_energies.sort(key=lambda x: x[2])

    writer = Chem.SDWriter(output_sdf)
    for mol, conf_id, energy in conformer_energies:
    ...
    writer.close()
    print(f"Sorted conformers saved to {output_sdf}")